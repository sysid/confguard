...
The confguard project implements a regression
...
not broken existing functionality. The confguard
...
``confguard`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``confguard`` package can be found.

.. code-block:: console

    (confguard) $ cd tests
    (confguard) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (confguard) $ python -m test_version
...
.. code-block:: console
    ...
    (confguard) $ make check-static-analysis
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/confguard/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (confguard) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (confguard) $ make dist-upload

  The package should now be available at https://pypi.org/project/confguard/
